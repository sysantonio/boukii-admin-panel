                ...
                this.defaults.settings.periods.forEach((period, periodIdx) => {
                  ...
                  this.defaults.course_dates.forEach(element => {
                    ...
                    if (current.isSame(from) || current.isSame(to) || current.isBetween(from, to)) {
                      this.dataSourceDatePrivate.data.push({ dateFrom: moment(element.date).format('YYYY-MM-DD'), dateTo: moment(element.date).format('YYYY-MM-DD'), active: element.active, id: element.id, main: false, period: periodIdx })
                    }
                  })
                });
...
/*        if (element.active || element.active === 1) {
  ...
  openDialogPrivateDate(): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.defaults.settings.periods.push({ from: moment(result.dateFrom).format('YYYY-MM-DD'), to: moment(result.dateTo).format('YYYY-MM-DD'), active: true })
        this.dataSourceDatePrivate.data.push({ dateFrom: moment(result.dateFrom).format('YYYY-MM-DD'), dateTo: moment(result.dateTo).format('YYYY-MM-DD'), active: true });
        this.privateDatesTable?.renderRows();
        this.activityDatesTable?.renderRows();
